...
def write_rtdb_vars_h(variables):
    ...
    with open('include/rtdb_vars.h', 'w') as file:
        ...
        file.write("typedef enum\n")
        file.write("{\n")
        file.write("    SIGNAL_ERROR = 0x00,         ///< Value unreliable\n")
        file.write("    SIGNAL_MISSING = 0x0F,       ///< Value missing\n")
        file.write("    SIGNAL_CAN_TIMEOUT = 0x3F,   ///< Timeout on CAN signal\n")
        file.write("    SIGNAL_OK = 0xFF             ///< Value reliable, use value\n")
...
def write_rtdb_c(variables, modules):
    with open('src/rtdb/rtdb.cpp', 'w') as file:
        ...
        for var_type in at:
            ...
            file.write('    {\n')
            file.write(f'        errh_reportError(ERRH_ERROR_HIGH, MODULE_RTDB, 0, RTDB_API_CALC_CRC_{to_all_caps(var_type)}, ERRH_ERR_ACCESS_NULL_PTR_U32);\n')
            file.write('    }\n')
            file.write('    else\n')
            file.write('    {\n')
        ...
        for var_type in at:
            ...
            file.write('    {\n')
            file.write(f'        errh_reportError(ERRH_WARNING, MODULE_RTDB, VarName, RTDB_API_OVERWRITE_{to_all_caps(var_type)}, RTDB_ERR_SEMAPH_TAKE_{to_all_caps(var_type)});\n')
            file.write('    }\n')
            file.write(f'    else\n')
            file.write('    {\n')
            ...
            file.write('        {\n')
            file.write(f'            errh_reportError(ERRH_WARNING, MODULE_RTDB, VarName, RTDB_API_WRITE_{to_all_caps(var_type)}, ERRH_ERR_READ_INDEX_OUT_OF_BOUNDS_U32);\n')
            ...
            file.write('        {\n')
            file.write(f'            errh_reportError(ERRH_WARNING, MODULE_RTDB, VarName, RTDB_API_WRITE_{to_all_caps(var_type)}, RTDB_ERR_WRONG_CRC_{to_all_caps(var_type)});\n')
            file.write('            stsCode = 2;\n')
            file.write('        }\n')
            file.write(f'        // check for MIN\n')
            file.write(f'        else if ( NewVal < rtdb_arr_{var_type}[VarName]->{bt[var_type]}_min)\n')
            file.write('        {\n')
            file.write(f'            errh_reportError(ERRH_WARNING, MODULE_RTDB, VarName, RTDB_API_WRITE_{to_all_caps(var_type)}, RTDB_ERR_VAL_BELOW_MIN_{to_all_caps(var_type)});\n')
            file.write(f'            stsCode = 3;\n')
            file.write('        }\n')
            file.write(f'        // check for MAX\n')
            file.write(f'        else if ( NewVal > rtdb_arr_{var_type}[VarName]->{bt[var_type]}_max)\n')
            file.write('        {\n')
            file.write(f'            errh_reportError(ERRH_WARNING, MODULE_RTDB, VarName, RTDB_API_WRITE_{to_all_caps(var_type)}, RTDB_ERR_VAL_ABOVE_MAX_{to_all_caps(var_type)});\n')
            ...
            file.write(f'                rtdb_arr_{var_type}[VarName]->ss.signalState = SIGNAL_OK;\n')
        ...
        for var_type in at:
            ...
            file.write('        {\n')
            file.write(f'            errh_reportError(ERRH_WARNING, MODULE_RTDB, VarName, RTDB_API_OVERWRITE_{to_all_caps(var_type)}, RTDB_ERR_WRONG_CRC_{to_all_caps(var_type)});\n')
            file.write('            stsCode = 1;\n')
            file.write('        }\n')
            file.write(f'        else\n')
            file.write('        {\n')
        ...
        for var_type in at:
            ...
            file.write(f'    if (!rtdb_lock())\n')
            file.write('    {\n')
            file.write(f'        errh_reportError(ERRH_WARNING, MODULE_RTDB, VarName, RTDB_API_RELEASE_OVERWRITE_{to_all_caps(var_type)}, RTDB_ERR_SEMAPH_TAKE_{to_all_caps(var_type)});\n')
            file.write('    }\n')
            file.write(f'    else\n')
            file.write('    {\n')
            file.write(f'        if (NUM_OF_{var_type} <= VarName)\n')
            file.write('        {\n')
            file.write(f'            errh_reportError(ERRH_WARNING, MODULE_RTDB, VarName, RTDB_API_RELEASE_OVERWRITE_{to_all_caps(var_type)}, ERRH_ERR_READ_INDEX_OUT_OF_BOUNDS_U32);\n')
            file.write('            stsCode = 1;\n')
            file.write('        }\n')
            file.write(f'        // check CRC\n')
            file.write(f'        else if (rtdb_checkCrc32_{var_type}(rtdb_arr_{var_type}[VarName], sizeof({var_type})))\n')
            file.write('        {\n')
            file.write(f'            errh_reportError(ERRH_WARNING, MODULE_RTDB, VarName, RTDB_API_RELEASE_OVERWRITE_{to_all_caps(var_type)}, RTDB_ERR_WRONG_CRC_{to_all_caps(var_type)});\n')
            file.write('            stsCode = 2;\n')
            file.write('        }\n')
            file.write(f'        else\n')
            file.write('        {\n')
        ...
        for var_type in at:
            ...
            file.write(f'    if (!rtdb_lock())\n')
            file.write('    {\n')
            file.write(f'        errh_reportError(ERRH_WARNING, MODULE_RTDB, VarName, RTDB_API_READ_{to_all_caps(var_type)}, RTDB_ERR_SEMAPH_TAKE_{to_all_caps(var_type)});\n')
            file.write('    }\n')
            file.write(f'    else\n')
            file.write('    {\n')
            file.write(f'        if (NUM_OF_{var_type} <= VarName)\n')
            file.write('        {\n')
            file.write(f'            errh_reportError(ERRH_WARNING, MODULE_RTDB, VarName, RTDB_API_READ_{to_all_caps(var_type)}, ERRH_ERR_READ_INDEX_OUT_OF_BOUNDS_U32);\n')
            file.write('        }\n')
            file.write(f'        // Check CRC\n')
            file.write(f'        else if (rtdb_checkCrc32_{var_type}(rtdb_arr_{var_type}[VarName], sizeof({var_type})))\n')
            file.write('        {\n')
            file.write(f'            errh_reportError(ERRH_WARNING, MODULE_RTDB, VarName, RTDB_API_READ_{to_all_caps(var_type)}, RTDB_ERR_WRONG_CRC_{to_all_caps(var_type)});\n')
            file.write('        }\n')
            file.write(f'        else\n')
            file.write('        {\n')
        ...
        for var_type in at:
            file.write(f'\nvoid rtdb_setSS_{var_type}( {var_type}EnumT VarName , signalStateT newSS)\n')
            file.write('{\n')
            file.write(f'    if (!rtdb_lock())\n')
            file.write('    {\n')
            file.write(f'        errh_reportError(ERRH_WARNING, MODULE_RTDB, VarName, RTDB_API_SETSS_{to_all_caps(var_type)}, RTDB_ERR_SEMAPH_TAKE_{to_all_caps(var_type)});\n')
            file.write('    }\n')
            file.write(f'    else\n')
            file.write('    {\n')
            file.write(f'        if (NUM_OF_{var_type} <= VarName)\n')
            file.write('        {\n')
            file.write(f'            errh_reportError(ERRH_WARNING, MODULE_RTDB, VarName, RTDB_API_SETSS_{to_all_caps(var_type)}, ERRH_ERR_READ_INDEX_OUT_OF_BOUNDS_U32);\n')
            file.write('        }\n')
            file.write(f'        // check CRC\n')
            file.write(f'        else if (rtdb_checkCrc32_{var_type}(rtdb_arr_{var_type}[VarName], sizeof({var_type})))\n')
            file.write('        {\n')
            file.write(f'            errh_reportError(ERRH_WARNING, MODULE_RTDB, VarName, RTDB_API_SETSS_{to_all_caps(var_type)}, RTDB_ERR_WRONG_CRC_{to_all_caps(var_type)});\n')
            file.write('        }\n')
            file.write(f'        else\n')
            file.write('        {\n')
            file.write(f'            if (OBJECT_STANDARD == rtdb_arr_{var_type}[VarName]->ss.objectStatus)\n')
            file.write('            {\n')
            file.write(f'                rtdb_arr_{var_type}[VarName]->ss.signalState = newSS;\n')
            file.write(f'                rtdb_arr_{var_type}[VarName]->ss.crc32 = rtdb_calcCrc32_{var_type}(rtdb_arr_{var_type}[VarName], sizeof({var_type}));\n')
            file.write('            }\n')
            file.write('        }\n')
            file.write('        rtdb_unlock();\n')
        ...
        file.write('    {\n')
        file.write(f'        errh_reportError(ERRH_NOTIF, MODULE_RTDB, 0, RTDB_API_INIT_U32, ERRH_MODULE_ALREADY_INIT);\n')
        file.write('    }\n')
        file.write('    else if (NULL == RtdbCfg)\n')
        file.write('    {\n')
        file.write(f'        errh_reportError(ERRH_ERROR_CRITICAL, MODULE_RTDB, 0, RTDB_API_INIT_U32, ERRH_POINTER_IS_NULL);\n')
        file.write('    }\n')
        file.write('    else\n')
        file.write('    {\n')
...
def write_rtdb_h(modules):
    with open('include/rtdb.h', 'w') as file:
        ...
        for var_type in at:
            ...
            var_indx = var_indx + 1
        for var_type in at:
            printable = f'#define RTDB_API_SETSS_{to_all_caps(var_type)}'
            while len(printable) < 50:
                printable += ' '
            printable += f'((uint32_t) {var_indx})\n'
            file.write(printable)
